...
**Windows 설치형 NSIS 인스톨러**(`Claude-Harness-Manager-Setup-x.y.z.exe`)와 **Linux AppImage**(x64 `Claude-Harness-Manager-x.y.z-x64.AppImage` + Jetson/라즈베리파이4+ 등 aarch64용 `Claude-Harness-Manager-x.y.z-arm64.AppImage`)로 배포한다 — 파일명에 공백을 쓰면 GitHub 자산명이 점으로 치환돼 updater가 404를 만나므로 `artifactName`으로 하이픈 고정(x64/arm64 자산이 같은 릴리스에서 같은 이름으로 충돌하지 않게 `${arch}`도 포함). (macOS는 서명/공증·빌드 환경 확보 후 별도 추가 예정으로 현재 제외.) 저장소가 **public**이라 익명 자동 업데이트(electron-updater)가 가능하다 — 앱이 기동 시·"업데이트 확인" 시 GitHub 릴리스를 토큰 없이 조회해 새 버전을 **배경에서 차등 다운로드**(blockmap delta — NSIS·AppImage 모두)하고, 완료되면 사이드바 배지가 **"재시작하여 적용"** 으로 바뀌어 `quitAndInstall(true, true)`로 **무음(`/S`) 설치 후 앱 자동 재실행**해 갈아끼운다(어시스트 인스톨러지만 업데이트는 인스톨러 창 없이 무음으로 돌아 "재설치하는 느낌"을 없앤다 — `isSilent=true`면 재실행 인자가 `isForceRunAfter`로 바뀌므로 두 번째 `true` 필수, 생략 시 설치 후 앱이 안 켜짐; `autoInstallOnAppQuit`로 종료 시 자동 설치도 보장). 받는 앱은 OS/아키텍처별 매니페스트(`latest.yml`/`latest-linux.yml`/`latest-linux-arm64.yml`)를 익명 조회한다(arm64 앱은 `latest-linux-arm64.yml`을 본다). 사용자는 수동 재설치를 하지 않는다. 자동 확인이 실패하면 배지가 GitHub 릴리스 페이지를 여는 fallback 링크를 제공한다. 다만 **각 플랫폼의 첫 자동업데이트 릴리스는 1회 수동 설치/다운로드**가 필요하다(Windows v1.0.0은 이전 v0.4.0에 updater 부재, Linux는 갈아끼울 기존 설치본 부재).
...
npm run dist         # build + electron-builder --win --publish never → release/에 setup.exe + latest.yml + blockmap 생성(로컬, gh로 수동 업로드 fallback)
npm run dist:linux   # build + electron-builder --linux --publish never → release/에 AppImage + latest-linux.yml + blockmap (Linux/WSL/Docker 환경 필요)
```
- **정식 릴리스는 GitHub Actions CI**(`.github/workflows/release.yml`)가 담당한다. `v*` 태그를 푸시하면 ubuntu·windows 러너가 OS별로 빌드해 같은 태그의 **draft 릴리스에 자산을 append**하고(`--publish always`), 사람이 GitHub에서 검토 후 수동 publish한다. 위 `dist`/`dist:linux`는 로컬 검증·수동 업로드 fallback용.
...
- **플랫폼 분기 코드**: `cc-detect.ts`는 `process.platform`으로 Windows는 `tasklist`(claude.exe), macOS/Linux는 `pgrep -x claude`로 CC 프로세스를 감지한다. `projects.ts`의 `guessOriginalPath`는 `process.platform`으로 분기 — Windows는 flatten된 디렉토리명(`D--hdx-agv`)을 `D:\hdx\agv`로, Unix는 선행 `-`(루트 구분자)를 살려 `-home-user-proj`를 `/home/user/proj`로 역추정한다(둘 다 구분자 `-`/이름 `-` 구별 불가 → 추정값; recall이 transcript `cwd`로 계산하는 `realPath`가 우선이고 이건 fallback). (배포 타깃은 Windows NSIS + Linux AppImage.)
...
- **배포(GitHub Actions CI + 자동 업데이트)**: 저장소가 public이라 electron-updater 자동 업데이트를 쓴다(`build.publish`는 github/rafaam11). 정식 릴리스는 **`v*` 태그 푸시 → CI(`.github/workflows/release.yml`)**가 ubuntu(x64)·ubuntu-24.04-arm(arm64)·windows 매트릭스로 빌드해(`npx electron-builder --linux | --linux --arm64 | --win --publish always`, `GH_TOKEN`=`GITHUB_TOKEN`) 같은 태그의 **draft 릴리스에 자산을 append**한다 — Windows 3종(`...Setup-x.y.z.exe`+`latest.yml`+`*.blockmap`), Linux x64 3종(`...x.y.z-x64.AppImage`+`latest-linux.yml`+`*.blockmap`), Linux arm64 3종(`...x.y.z-arm64.AppImage`+`latest-linux-arm64.yml`+`*.blockmap` — Jetson/라즈베리파이4+ aarch64용, 무료 arm64 러너는 public repo 한정). 세 잡이 같은 draft를 채운 뒤 **사람이 GitHub에서 검토하고 수동 publish**한다(반쪽 릴리스 노출·레이스 방지 게이트). CI가 OS별 매니페스트/blockmap을 자동 생성하므로 수동 업로드 시절의 "latest.yml/blockmap 누락" 풋건이 사라진다. 로컬 fallback은 `npm run dist`(Windows)·`npm run dist:linux`(Linux) + `gh release upload`. 받는 앱은 토큰 없이 릴리스를 익명 조회해 새 버전을 배경 다운로드하고 "재시작하여 적용"으로 갈아끼운다. Windows는 코드서명이 없어 설치 시 SmartScreen 경고가 뜨고(추가 정보 → 실행), Linux AppImage는 서명 불필요(실행 권한만 부여). **각 플랫폼·아키텍처 첫 릴리스는 1회 수동 설치/다운로드 필요**(Windows v1.0.0은 이전 v0.4.0에 updater 부재, Linux x64·arm64는 각각 갈아끼울 기존 설치본 부재). dev에선 `app.isPackaged=false`라 updater가 비활성(`dev-app-update.yml`은 dev 테스트용 좌표, github 좌표라 OS 무관).
