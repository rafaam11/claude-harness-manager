...
**Windows 설치형 NSIS 인스톨러**(`Claude-Harness-Manager-Setup-x.y.z.exe`)와 **Linux AppImage**(`Claude-Harness-Manager-x.y.z.AppImage`)로 배포한다 — 파일명에 공백을 쓰면 GitHub 자산명이 점으로 치환돼 updater가 404를 만나므로 `artifactName`으로 하이픈 고정. (macOS는 서명/공증·빌드 환경 확보 후 별도 추가 예정으로 현재 제외.) 저장소가 **public**이라 익명 자동 업데이트(electron-updater)가 가능하다 — 앱이 기동 시·"업데이트 확인" 시 GitHub 릴리스를 토큰 없이 조회해 새 버전을 **배경에서 차등 다운로드**(blockmap delta — NSIS·AppImage 모두)하고, 완료되면 사이드바 배지가 **"재시작하여 적용"** 으로 바뀌어 `quitAndInstall`로 갈아끼운다(`autoInstallOnAppQuit`로 종료 시 자동 설치도 보장). 받는 앱은 OS별 매니페스트(`latest.yml`/`latest-linux.yml`)를 익명 조회한다. 사용자는 수동 재설치를 하지 않는다. 자동 확인이 실패하면 배지가 GitHub 릴리스 페이지를 여는 fallback 링크를 제공한다. 다만 **각 플랫폼의 첫 자동업데이트 릴리스는 1회 수동 설치/다운로드**가 필요하다(Windows v1.0.0은 이전 v0.4.0에 updater 부재, Linux는 갈아끼울 기존 설치본 부재).
...
- **배포(GitHub Actions CI + 자동 업데이트)**: 저장소가 public이라 electron-updater 자동 업데이트를 쓴다(`build.publish`는 github/rafaam11). 정식 릴리스는 **`v*` 태그 푸시 → CI(`.github/workflows/release.yml`)**가 ubuntu·windows 매트릭스로 빌드해(`npx electron-builder --linux|--win --publish always`, `GH_TOKEN`=`GITHUB_TOKEN`) 같은 태그의 **draft 릴리스에 자산을 append**한다 — Windows 3종(`...Setup-x.y.z.exe`+`latest.yml`+`*.blockmap`), Linux 3종(`...x.y.z.AppImage`+`latest-linux.yml`+`*.blockmap`). 두 잡이 같은 draft를 채운 뒤 **사람이 GitHub에서 검토하고 수동 publish**한다(반쪽 릴리스 노출·레이스 방지 게이트). CI가 OS별 매니페스트/blockmap을 자동 생성하므로 수동 업로드 시절의 "latest.yml/blockmap 누락" 풋건이 사라진다. 로컬 fallback은 `npm run dist`(Windows)·`npm run dist:linux`(Linux) + `gh release upload`. 받는 앱은 토큰 없이 릴리스를 익명 조회해 새 버전을 배경 다운로드하고 "재시작하여 적용"으로 갈아끼운다. Windows는 코드서명이 없어 설치 시 SmartScreen 경고가 뜨고(추가 정보 → 실행), Linux AppImage는 서명 불필요(실행 권한만 부여). **각 플랫폼 첫 릴리스는 1회 수동 설치/다운로드 필요**(Windows v1.0.0은 이전 v0.4.0에 updater 부재, Linux는 기존 설치본 부재). dev에선 `app.isPackaged=false`라 updater가 비활성(`dev-app-update.yml`은 dev 테스트용 좌표, github 좌표라 OS 무관).
