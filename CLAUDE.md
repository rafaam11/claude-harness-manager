...
Claude Code 전역 환경(`~/.claude` 디렉토리 + `~/.claude.json`)을 관리하는 로컬 전용 웹 대시보드. 스킬/에이전트/플러그인/프로젝트 카탈로그 조회, 설정 파일 편집, 규칙 기반 정리(아카이브)를 제공한다. 사용자 자신의 머신에서만 동작하며 외부로 노출되지 않는 것을 전제로 설계됐다.
...
1. **경로 allowlist** (`server/src/lib/path-guard.ts`) — 모든 파일 연산은 `guardPath()`로 경로를 resolve한 뒤 `ALLOWED_ROOTS`(`~/.claude`, `~/.claude.json`) prefix 검사를 통과해야 한다. 통과 못 하면 `PathViolationError`(403).
...
- `src/config.ts`: 모든 경로 상수·정책 값의 단일 출처(`ALLOWED_ROOTS`, `CONFIG_FILES`, `STALE_DAYS=30`, `AGENT_SIZE_WARN_BYTES=19KB`, `BACKUP_KEEP=20`, `TEMP_DIRS`).
- `src/services/`: 읽기 전용 도메인 로직 — `catalog`(skills/agents/commands 프론트매터 파싱), `plugins`(installed_plugins·blocklist·settings 병합), `projects`(프로젝트 디렉토리 walk·stale 계산), `scan`(정리 후보 산출).

### 웹 (`web/`, React 18 + Vite, 의존성 최소)

- 라우터·상태관리 라이브러리 없음. `App.tsx`가 `useState`로 탭 5개(`PAGES`)를 직접 전환한다.
- `src/api/client.ts`: `fetch` 래퍼(`api.get/put/post`) + `ApiError`(서버 `{error}` 본문을 던짐) + `fmtSize`/`fmtDate` 포맷터. 모든 페이지가 이걸 통해 API 호출.
- 페이지별 책임: `Overview`(요약 카드), `Catalog`(카탈로그·플러그인), `Memory`(프로젝트 파일 읽기 전용 브라우저), `Cleanup`(스캔→체크박스 선택→**dry-run 먼저**→실행→manifest 복구), `ConfigEditor`(설정 편집).

### 설정 파일 정책 (`CONFIG_FILES`)

- `settings.json`, `settings.local.json`은 **편집 가능**(`writable: true`), `~/.claude.json`은 **읽기 전용**(Claude Code가 상시 재작성하므로 충돌 방지 — 수동 절차로만 수정). 쓰기 라우트는 `writable: false`면 403.
...
- **Windows 전제 코드**가 있다: `cc-detect.ts`는 `tasklist`로 `claude.exe`를 찾고, `projects.ts`의 `guessOriginalPath`는 flatten된 디렉토리명(`D--hdx-agv`)을 `D:\hdx\agv`로 역추정한다(구분자 `-`/이름 `-` 구별 불가 → 추정값).
