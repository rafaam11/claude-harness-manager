// News 탭 IPC 계약. main(services/news.ts)이 세 소스를 fetch·정규화한 응답 형태와 1:1 대응한다.
// 분량상 별도 파일로 분리하고 types.ts가 re-export한다(git-types.ts 선례).

/** 뉴스 소스 식별자. UI 배지·소스별 상태에 쓴다. */
export type NewsSource = "claude-code" | "anthropic" | "ai";

/**
 * 세 소스를 시각 역순으로 병합하기 위한 정규화 항목.
 * - claude-code: GitHub 릴리스. body(패치노트 마크다운)를 앱 내에서 렌더.
 * - anthropic / ai: 제목+메타만, 원문은 OS 브라우저로 연다.
 */
export interface NewsItem {
  /** 소스별 안정 키(cc-<tag> / anthropic-<slug> / hn-<objectID>). React key + 펼침 키. */
  id: string;
  source: NewsSource;
  title: string;
  /** 원문 절대 URL(anthropic은 base + 상대경로로 절대화). 원문 열기 대상. */
  url: string;
  /** published_at/created_at의 epoch ms. 병합 정렬 키. */
  timestamp: number;
  /** claude-code 릴리스 패치노트 마크다운(앱 내 marked 렌더). 다른 소스는 없음. */
  body?: string;
  /** 보조 표시(예: HN "▲52 · 💬13", anthropic 카테고리). 선택. */
  meta?: string;
}
...
export interface NewsFeed {
  version: 1;
  items: NewsItem[];
  /** 길이 3 고정(claude-code/anthropic/ai 순). */
