// News 탭 IPC 계약. main(services/news.ts)이 다섯 소스를 fetch·정규화한 응답 형태와 1:1 대응한다.
// 분량상 별도 파일로 분리하고 types.ts가 re-export한다(git-types.ts 선례).

/** 뉴스 소스 식별자. UI 배지·소스별 상태에 쓴다. geeknews/aitimes/yozm은 한국어 RSS 소스. */
export type NewsSource = "claude-code" | "anthropic" | "geeknews" | "aitimes" | "yozm";

/**
 * 다섯 소스를 시각 역순으로 병합하기 위한 정규화 항목.
 * - claude-code: GitHub 릴리스. body(패치노트 마크다운)를 앱 내에서 렌더.
 * - anthropic: 제목만, 원문은 OS 브라우저로 연다.
 * - geeknews / aitimes / yozm: 한국어 RSS. summary(평문 발췌)를 앱 내에서 표시.
 */
export interface NewsItem {
  /** 소스별 안정 키(cc-<tag> / anthropic-<slug> / gn-<id> / at-<idxno> / yozm-<path>). React key + 펼침 키. */
  id: string;
  source: NewsSource;
  title: string;
...
export interface NewsFeed {
  version: 1;
  items: NewsItem[];
  /** 길이 5 고정(claude-code/anthropic/geeknews/aitimes/yozm 순). */
